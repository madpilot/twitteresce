...
public class DeleteDirectMessageThread extends Thread  {
	private Twitteresce parent;
	private TwitterAPI api;
	private int id;
	
	public DeleteDirectMessageThread(int id, Twitteresce parent) {
		this.id = id;
		this.parent = parent;
	...
	public void run() {
		...
		try {
			Alert deleting = new Alert("Please wait", "Deleting message...", null, AlertType.INFO);
			...
			this.api.DestroyDirectMessage(this.id);
			...
			Alert error = new Alert("Can't delete message", "Twitter is unavailable, please try again later", null, AlertType.ERROR);
			
			if(hie.getHttpCode() == HttpConnection.HTTP_FORBIDDEN || hie.getHttpCode() == HttpConnection.HTTP_UNAUTHORIZED)
			{
				error = new Alert("You aren't allowed to delete direct messages", "It looks like your username or password is incorrect", null, AlertType.ERROR);
			}
			...
			Alert error = new Alert("Can't delete the message", "Please try again later", null, AlertType.ERROR);
